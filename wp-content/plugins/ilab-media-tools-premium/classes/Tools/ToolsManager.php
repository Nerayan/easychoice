...
//        MigrationsManager::instance()->displayMigrationErrors();
	...
	public function addMenus($networkMode, $networkAdminMenu) {
		...
		foreach($this->tools as $key => $tool)  {
			...
			if ($key != 'troubleshooting') {
			    $tool->registerMenu('media-cloud', $networkMode, $networkAdminMenu, ($this->isLocal && StorageToolSettings::instance()->useToolMenu) ? 'media-cloud-tools' : 'media-cloud');
            }
		...
		add_submenu_page('media-cloud', 'Documentation', 'Documentation', 'manage_options', 'https://support.mediacloud.press/');
		if (media_cloud_licensing()->is_plan('pro')) {
			add_submenu_page('media-cloud', 'Submit Issue', 'Submit Issue', 'manage_options', 'https://support.mediacloud.press/submit-issue/');
		}
