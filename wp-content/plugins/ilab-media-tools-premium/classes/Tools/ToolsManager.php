...
final class ToolsManager {
    ...
    public function __construct() {
	    ...
	    if ($runTime == 0) {
		    ...
		    NoticeManager::instance()->displayAdminNotice('info',"Thanks for using Media Cloud!  If you like it, please <a href='https://wordpress.org/support/plugin/ilab-media-tools/reviews/#new-post' target=_blank>leave a review</a>.  Thank you!", true,'ilab-media-tools-nag-notice');
        }
    ...
    public static function Boot() {
        ...
        if (!is_multisite() && ($media_cloud_licensing->is_plan__premium_only('multisite_basic', true) || $media_cloud_licensing->is_plan__premium_only('multisite_pro', true) || $media_cloud_licensing->is_plan__premium_only('multisite_unlimited', true))) {
		    ...
		    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
			    ToolsManager::registerTool("glide", include ILAB_CONFIG_DIR . '/glide.config.php');
		    }
	...
	public function addMenus($networkMode, $networkAdminMenu) {
		...
		if ($media_cloud_licensing->is_plan__premium_only('pro')) {
			$isNetworkModeAdmin = ($networkMode && $networkAdminMenu);
			$isLocal = (!$networkMode && !$networkAdminMenu);
		} else {
			$networkMode = false;
		...
		if ($isLocal || $isNetworkModeAdmin) {
			if (!empty($this->pinnedTools)) {
				...
				foreach($this->pinnedTools as $pinnedTool => $value) {
					...
					add_submenu_page( 'media-cloud', $tool->toolInfo['name'].' Settings', $tool->toolInfo['name'], 'manage_options', 'media-cloud-settings&tab='.$pinnedTool, [$this,'renderSettings']);
		...
		if ($isLocal || $isNetworkModeAdmin) {
			...
			add_submenu_page('media-cloud', 'Plugin Support', 'Support', 'manage_options', 'https://talk.mediacloud.press/');
			...
			if (LicensingManager::CanTrack()) {
				add_submenu_page('media-cloud', 'Opt-In Settings', 'Opt-In Settings', 'manage_options', admin_url('admin.php?page=media-cloud-settings&tab=opt-in'));
			}
		}
    ...
    public function insertSeparator($title = '') {
        if (empty($title)) {
            add_submenu_page( 'media-cloud', '', '<span class="ilab-admin-separator-container"><span class="ilab-admin-separator"></span></span>', 'manage_options', '#');
        } else {
            $safe_title = sanitize_title($title);
            add_submenu_page( 'media-cloud', '', '<span class="ilab-admin-separator-container ilab-admin-separator-'.$safe_title.'"><span class="ilab-admin-separator-title">'.$title.'</span><span class="ilab-admin-separator"></span></span>', 'manage_options', '#');
        }
    }
    ...
    public function renderSettings() {
	    ...
	    if (!is_multisite() && ($media_cloud_licensing->is_plan__premium_only('multisite_basic', true) || $media_cloud_licensing->is_plan__premium_only('multisite_pro', true) || $media_cloud_licensing->is_plan__premium_only('multisite_unlimited', true))) {
		    ...
		    foreach((array)$wp_settings_sections[$page] as $section) {
			    ...
			    $sections[] = ['title' => $section['title'], 'id' => $section['id'], 'help' => $help, 'description' => arrayPath($selectedTool->toolInfo, "settings/groups/{$section['id']}/description", null)];
		    }

		    echo View::render_view('base/settings', ['title' => 'All Settings', 'tab' => $tab, 'tools' => $this->tools, 'tool' => $selectedTool, 'group' => $group, 'page' => $page, 'manager' => $this, 'sections' => $sections]);
	    }
    }
