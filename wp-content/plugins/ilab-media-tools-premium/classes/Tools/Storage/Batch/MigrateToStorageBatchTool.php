...
class MigrateToStorageBatchTool extends BatchTool {
    ...
    public function handleBulkActions($redirect_to, $action_name, $post_ids) {
        if('ilab_s3_import' === $action_name) {
        	$result = $this->processBulkSelection($post_ids, true);
        	if (!empty($result)) {
        		return $result;
	        }
        }
	...
	protected function getImportBatch($page, $forceImages = false, $allInfo = false) {
		...
		$result['options'] = [
			...
			'ignore-mime-types' => !empty($_REQUEST['ignore-mime-types'])
		];
    ...
    public function manualAction() {
	    ...
	    if (!empty($_REQUEST['ignore-mime-types'])) {
		    add_filter('media-cloud/storage/ignore-mime-types', function($ignore) {
			    return false;
		    });
	    }
