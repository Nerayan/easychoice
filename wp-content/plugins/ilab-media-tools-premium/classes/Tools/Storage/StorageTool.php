...
class StorageTool extends Tool {
    ...
    public function handleUpdateAttachmentMetadataFor53($data, $id) {
	    ...
	    if (!wp_attachment_is_image($id)) {
		    Logger::info("Attachment is not an image $id", [], __METHOD__, __LINE__);
		    return $data;
	    }
    ...
    public function updateAttachmentMetadata($data, $id, $preserveFilePaths = 'replace', $skipThumbnails = false, $existingPrefix = null) {
	    ...
	    if ($media_cloud_licensing->is_plan__premium_only('basic')) {
		    if(ToolsManager::instance()->toolEnabled('optimizer')) {
                ...
                if ($skipThumbnails && isset($data['sizes'])) {
                    ...
                    foreach($data['sizes'] as $key => $size) {
                        ...
                        if (($file == $data['file']) || ($sizedFileName == $data['file'])) {
                            ...
                            $sizeData = $this->processFile($upload_path, $file, $size, $id, $preserveFilePaths, true, null, $existingPrefix, $key);
		...
		if($this->client && $this->client->enabled()) {
			...
			if(($upload['type'] == 'application/pdf') && file_exists($upload_path.'/'.$file) && function_exists('gzuncompress')) {
				...
				try {
					...
					Logger::error('PDF Parsing Error', ['exception' => $ex->getMessage()], __METHOD__, __LINE__);
				}
	...
	public function filterGutenbergContent($content) {
        ...
        if ((count($galleryElements) === 2) && !empty($galleryElements[1])) {
	        ...
	        preg_match_all('/<li\s+(?:[^>]*)blocks-gallery-item(?:[^>]+)>\s*<figure\s*(?:[^>]*)>\s*(<a[^>]+>)\s*(<img[^>]+>)\s*<\/a>\s*<\/figure>\s*<\/li>/m', $content, $galleryElements, PREG_SET_ORDER, 0);
    ...
    private function processFileBlock($id, $block_content) {
	    if(preg_match_all('/<a\s+([^>]*)>/m', $block_content, $anchors, PREG_SET_ORDER, 0)) {
	...
	private function processAudioBlock($id, $block_content) {
	    if(preg_match_all('/<audio\s+([^>]*)>/m', $block_content, $audioTags, PREG_SET_ORDER, 0)) {
	...
	private function processVideoBlock($id, $block_content) {
	    if(preg_match_all('/<video\s+([^>]*)>/m', $block_content, $videoTags, PREG_SET_ORDER, 0)) {
	...
	private function processCoverBlock($id, $block_content) {
		if(preg_match_all('/class\s*=\s*"([^"]*)/m', $block_content, $classes, PREG_SET_ORDER, 0)) {
		    foreach($classes as $class) {
		...
		try {
		    ...
		    if (isset($additionalPaths['file-webp'])) {
		        ...
		        if (!$this->client->exists($prefix.$webpBasename)) {
			        $this->client->upload($prefix.$webpBasename, $webpPath, $privacy, StorageToolSettings::cacheControl(), StorageToolSettings::expires(), 'image/webp');
		...
		if (count($this->deleteCache) > 0) {
			...
			if (StorageToolSettings::queuedDeletes()) {
				...
				foreach($this->deleteCache as $file) {
					@unlink($file);
					if (file_exists($file)) {
						Logger::info("StorageTool::cleanUploads - Unable to delete $file - maybe permissions issue?", [], __METHOD__, __LINE__);
					}
				}
			}
...
//
			...
			if (is_file($upload_file)) {
				...
				if ($mime == 'application/pdf') {
					...
					try {
						...
						Logger::error( 'PDF Exception.',  [ 'postId' => $postId, 'exception' =>$ex->getMessage()], __METHOD__, __LINE__);
					}
    ...
    //region File List
    public function getFileList($directoryKeys = [''], $skipThumbnails = false) {
	    $tempFileList = [];
