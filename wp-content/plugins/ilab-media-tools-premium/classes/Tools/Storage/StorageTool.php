...
class StorageTool extends Tool {
	...
	public function __construct($toolName, $toolInfo, $toolManager) {
		...
		if (media_cloud_licensing()->is_plan__premium_only('basic')) {
			new \ILAB\MediaCloud\Tools\Storage\Batch\MigrateToStorageBatchProcess();
			new \ILAB\MediaCloud\Tools\Storage\Batch\RegenerateThumbnailBatchProcess();
        }
	...
	public function handleUpload($upload, $context = 'upload') {
		...
		$shouldHandle = apply_filters_deprecated('ilab_s3_should_handle_upload', [true, $upload], '3.0.0', 'media-cloud/storage/should-handle-upload');
		...
		try {
		    ...
		    if (isset($additionalPaths['file-webp'])) {
		        ...
		        if (!$this->client->exists($prefix.$webpBasename)) {
			        $this->client->upload($prefix.$webpBasename, $webpPath, StorageSettings::privacy(), StorageSettings::cacheControl(), StorageSettings::expires(), 'image/webp');
			        if(StorageSettings::deleteOnUpload()) {
                        unlink($webpPath);
			        }
                }
...
//
	 ...
	 * @throws StorageException
	 */
	public function importAttachmentFromStorage($fileInfo, $thumbs = []) {
		if(!$this->client || !$this->client->enabled()) {
			return null;
		}
...
Optimizer;
	...
	public function importFileFromStorage($key, $thumbs, $importOnly, $preservePaths) {
