...
namespace ILAB\MediaCloud\Vision\Driver\GoogleCloudVision;
...
class GoogleCloudVisionDriver extends VisionDriver {
    ...
    private function processResults($meta, $postID, $results) {
        ...
        if (!empty($info['labelAnnotations'])) {
            $tags = $this->getTags($info['labelAnnotations'], ($this->settings->detectLabelsConfidence / 100.0));
        ...
        if (!empty($info['logoAnnotations'])) {
            $tags = $this->getTags($info['logoAnnotations'], ($this->settings->detectLabelsConfidence / 100.0));
        ...
        if (!empty($info['landmarkAnnotations'])) {
            $tags = $this->getTags($info['landmarkAnnotations'], ($this->settings->detectLabelsConfidence / 100.0));
        ...
        if (!empty($info['safeSearchAnnotation'])) {
            ...
            foreach($info['safeSearchAnnotation'] as $key => $safeSearch) {
                if ($this->getModerationLevel($safeSearch) >= $this->settings->detectExplicitConfidence) {
                    $tags[] = [
                        'tag' => $key
                    ];
                }
    ...
    private function getTags($annotations, $confidence) {
        ...
        foreach($annotations as $annotation) {
            if ($annotation['score'] > $confidence) {
                if (!in_array($annotation['description'], $this->settings->ignoredTags)) {
                    $tags[] = [
